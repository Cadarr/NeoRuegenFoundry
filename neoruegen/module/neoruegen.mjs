...
import { NeoruegenItem } from './documents/item.mjs';
// Import sheet classes.
import { NeoruegenActorSheet } from './sheets/actor-sheet.mjs';
import { NeoruegenItemSheet } from './sheets/item-sheet.mjs';
...
Hooks.once('init', function () {
  ...
  Items.registerSheet('neoruegen', NeoruegenItemSheet, {
    makeDefault: true,
    label: 'NEORUEGEN.SheetLabels.Item',
  });
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.neoruegen.rollItemMacro("${data.uuid}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { 'neoruegen.itemMacro': true },
    });
  }
