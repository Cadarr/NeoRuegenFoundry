...
class SystemGenerator {
   ...
   *
   * Replace refercnes to 'neoruegen', 'Neoruegen', and 'NEORUEGEN'
  ...
  replaceFileContents() {
    ...
    const replacements = [
      {
        pattern: new RegExp(/game\.neoruegen/g),
        replacement: `game.${this.propName}`
      },
      {
        pattern: new RegExp(/flags\.neoruegen/g),
   ...
   * Rename files.
   *
   * Rename files that had neoruegen in their name, such as
   * css/neoruegen.css.
   */
  renameFiles() {
    glob(`build/${this.packageName}/**/*neoruegen*.*`).then(files => {
      files.forEach(oldPath => {
        const file = path.basename(oldPath);
        const directory = path.dirname(oldPath);
        fs.rename(oldPath, `${directory}/${file.replaceAll('neoruegen', this.packageName)}`, (err) => {
          if (err) throw err;
        });
      })
    })
  ...
  cleanPackageJson() {
    ...
    fs.rmSync(`build/${this.packageName}/src/generate-neoruegen-system.mjs`);
...
inquirer
  ...
  .then((answers) => {
    ...
    // Glob Neoruegen's files so that we can process them.
    glob('*', {ignore: ['node_modules/**'] }).then(files => {
      // Copy all files into the build dir.
      generator.copyFiles(files);
      // Replace neoruegen name mentions in files.
      generator.replaceFileContents();
      // Rename files that had neoruegen in their name.
